...
router.get('/catalog', async (req, res) => {
    try {
        ...
        res.render('item/catalog', { items, title: 'All-Ads Page', error: createErrorMsg(null, sessionError) });    
    } catch (error) {
        return res.render('item/catalog', { title: 'All-Ads Page', error: createErrorMsg(error, null) });
    }
});
...
router.get('/search', async (req, res) => {
    res.render('item/search', { title: 'Search' }); 
});
...
router.post('/search', async (req, res) => {
    ...
    try {
        const user = await authService.getFilteredUser(query).lean();
    
        res.render('item/search', { searching: true, user, query, title: 'Search' }); 
    } catch (error) {
        return res.render('item/search', { title: 'Search', error: createErrorMsg(error, null)});
    }
});
...
router.get('/:itemId/details', async (req, res) => {
    ...
    try {
        ...
        res.render('item/details', { item, isOwner, isLiked, owner, title: 'Details Page' });
    } catch (error) {
